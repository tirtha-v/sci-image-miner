#!/usr/bin/env python3
"""Dev/test inference for LoRA-finetuned Qwen2.5-VL-7B-Instruct.

For dev set, reads from data/dev_panels.csv (pre-cropped panels).
For test set, walks competition JSON directory structure.

Usage:
    python run_qwen_qlora_predict.py \
        --adapter-path outputs/vlm_finetune/qwen2vl_qlora/checkpoint-XXX \
        --panels-csv data/dev_panels.csv \
        --output-dir outputs/qwen2vl_qlora_ft/dev \
        --device cuda:0
"""
...
def load_model(adapter_path: str, device: str):
    ...
    # Use device_map="balanced" to spread 14GB bfloat16 model across all
    # visible GPUs. Set CUDA_VISIBLE_DEVICES externally.
...
def run_predict(adapter_path: str, panels_csv: str, output_dir: str, device: str, with_descriptions: bool = False, selective_descriptions: bool = False):
    _, user_prompt = build_classification_prompt(with_descriptions=with_descriptions, selective_descriptions=selective_descriptions)
...
def run_predict_from_test_root(adapter_path: str, test_root: str, output_dir: str, device: str, with_descriptions: bool = False, selective_descriptions: bool = False):
    """Run inference by walking competition JSON structure (for test set)."""
    import glob
    import os
    _, user_prompt = build_classification_prompt(with_descriptions=with_descriptions, selective_descriptions=selective_descriptions)
    output_dir = Path(output_dir)
...
def main():
    ...
    parser.add_argument("--with-descriptions", action="store_true",
                        help="Include short descriptions for each category in the prompt")
    parser.add_argument("--selective-descriptions", action="store_true",
                        help="Include targeted hints only for confusable category pairs")
    args = parser.parse_args()

    if not args.panels_csv and not args.test_root:
        raise ValueError("Provide either --panels-csv (dev) or --test-root (test)")

    for hf_token_path in [
        "/home/jovyan/tvinchur/hf_token_for_cluster.txt",
        "/home/jovyan/shared-scratch-tvinchur-pvc/tvinchur/hf_token_for_cluster.txt",
    ]:
        hf_token_file = Path(hf_token_path)
    ...
    if args.panels_csv:
        run_predict(args.adapter_path, args.panels_csv, args.output_dir, args.device,
                    ...
                    selective_descriptions=args.selective_descriptions)
    else:
