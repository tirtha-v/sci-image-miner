#!/bin/bash
# =============================================================================
# Run VLM predictions for eval phase (requires A6000 GPUs)
# Run this on a node with A6000s, then run run_eval_phase.sh to rebuild zip.
#
...
HF_TOKEN_FILE="/home/jovyan/tvinchur/hf_token_for_cluster.txt"
...
CUDA_VISIBLE_DEVICES=0,1 $PYTHON run_llama_qlora_predict.py \
...
CUDA_VISIBLE_DEVICES=2,3 $PYTHON run_classify.py \
    --model qwen2vl \
    --test-root "$TEST_ROOT" \
