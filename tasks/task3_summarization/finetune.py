#!/usr/bin/env python3
"""QLoRA fine-tuning of Qwen2.5-VL-7B for summarization (Task 3).

Reads train_summarization.csv (image_path, sample_id, panel_id, summary).
Fine-tunes the model to output a 1-3 sentence scientific summary from a panel image.

Usage:
    CUDA_VISIBLE_DEVICES=0,1,2,3 python tasks/task3_summarization/finetune.py \
        --train-csv data/train_summarization.csv \
        --output-dir outputs/qwen2vl_summarization_qlora \
        --epochs 3 --lr 1e-4
"""
...
class SummarizationSFTDataset(Dataset):
...
def main():
    ...
    parser.add_argument("--train-csv", default="data/train_summarization.csv")
    parser.add_argument("--output-dir", default="outputs/qwen2vl_summarization_qlora")
    ...
    print(f"[Summarization QLoRA] Loading {args.model_id} ...")
    ...
    train_dataset = SummarizationSFTDataset(args.train_csv)
    ...
    trainer = Trainer(
        ...
        data_collator=SummarizationCollator(processor),
    )

    print("[Summarization QLoRA] Starting training...")
    trainer.train()

    adapter_path = output_dir / "adapter"
    model.save_pretrained(str(adapter_path))
    processor.save_pretrained(str(adapter_path))
    print(f"[Summarization QLoRA] Adapter saved to {adapter_path}")
